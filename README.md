...
| `state_file` | 신청 성공 이력 파일. 기본 `applied.json`. **계정마다 다르게 지정**한다 |
...
**계정을 바꿀 때는 반드시 이전 세션을 지워야 한다.** `ensure_login()` 은 살아있는
세션이 있으면 *어느 계정인지 확인하지 않고* 그대로 재사용한다. 그래서
`config.json` 의 아이디만 바꾸고 실행하면 새 자격증명은 무시되고
**이전 계정으로 신청이 들어간다.**

```bash
rm -f session.json          # 로그인 쿠키 (세션 쿠키라 여기에만 남는다)
rm -rf .chrome-profile      # 영속 프로필
```

신청 이력도 계정별로 분리한다. `applied.json` 은 `(코트, 날짜, 슬롯)` 만 기록하고
계정은 구분하지 않으므로, 한 파일을 공유하면 다른 계정에서 이미 신청했다는 이유로
건너뛰게 된다.

```json
{ "state_file": "applied.wltjs4374.json" }
```

## 로그인 5회 실패 시 잠금

비밀번호가 틀리면 `아이디 또는 패스워드가 틀렸습니다. 로그인 시도 : N/5` alert 가
뜬다. **5회 누적되면 계정이 잠기므로 추측으로 재시도하면 안 된다.** 입력이 제대로
들어갔는지는 제출 없이 확인할 수 있다 — 로그인 페이지에서 값을 채운 뒤
`input[name='user_id']` / `input[name='user_password']` 의 `value` 를 읽어
설정값과 비교하면 오타인지 자격증명 자체가 틀린 것인지 구분된다.
